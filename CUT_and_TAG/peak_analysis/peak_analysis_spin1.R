...
s1f <- list.files("../",pattern = "SPIN1_E145_NR_peaks.narrowPeak",full.names = T,recursive = F)
names(s1f) <- s1f |> str_remove("../") |> str_remove("_peaks.narrowPeak")

s1.gr <- readPeakFile(s1f)

## Read in individual peaks
s1f <- list.files("../",pattern = "SPIN1_E145_R[1-3]_NR_peaks.narrowPeak",full.names = T,recursive = F)
names(s1f) <- s1f |> str_remove("../") |> str_remove("_peaks.narrowPeak")
...
s1rc <- read_tsv("../SPIN1_E145_peaks_sum.NR.tsv",col_names = c("chr","start","end","SPIN1_E145_R1","SPIN1_E145_R2","SPIN1_E145_R3"),skip = 1) |>
