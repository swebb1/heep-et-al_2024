...
k4f <- list.files("../../bam_files/CUT_and_TAG_peaks",pattern = "H3K4me3_E145_all_peaks.narrowPeak",full.names = T,recursive = F)
names(k4f) <- k4f |> str_remove("../../bam_files/CUT_and_TAG_peaks/") |> str_remove("_peaks.narrowPeak")
k4.gr <- readPeakFile(k4f)

k9f <- list.files("../../bam_files/CUT_and_TAG_peaks",pattern = "H3K9me3_E145_all_peaks.narrowPeak",full.names = T,recursive = F)
names(k9f) <- k9f |> str_remove("../../bam_files/CUT_and_TAG_peaks/") |> str_remove("_peaks.narrowPeak")
k9.gr <- readPeakFile(k9f)

## Read in individual peaks
k4f <- list.files("../../bam_files/CUT_and_TAG_peaks",pattern = "H3K4me3_E145_R[1-2]_all_peaks.narrowPeak",full.names = T,recursive = F)
names(k4f) <- k4f |> str_remove("../../bam_files/CUT_and_TAG_peaks/") |> str_remove("_all_peaks.narrowPeak")
k4.grl <- k4f |> map(~readPeakFile(.x))
names(k4.grl) <- names(k4f)

k9f <- list.files("../../bam_files/CUT_and_TAG_peaks",pattern = "H3K9me3_E145_R[1-2]_all_peaks.narrowPeak",full.names = T,recursive = F)
names(k9f) <- k9f |> str_remove("../../bam_files/CUT_and_TAG_peaks/") |> str_remove("_all_peaks.narrowPeak")
...
k4rc <- read_tsv("../../bam_files/CUT_and_TAG_peaks/H3K4me3_E145_peaks_sum.all.tsv",col_names = c("chr","start","end","H3K4me3_E145_R1","H3K4me3_E145_R2"),skip = 1) |>
  mutate(start=start+1,length=end-start)

k9rc <- read_tsv("../../bam_files/CUT_and_TAG_peaks/H3K9me3_E145_peaks_sum.all.tsv",col_names = c("chr","start","end","H3K9me3_E145_R1","H3K9me3_E145_R2"),skip = 1) |>
  mutate(start=start+1,length=end-start)
